...
def test_a_pair_scores_fifty_at_the_average_and_projects_its_synergy_into_gold_at_15(serving_settings):
    # given
    settings = serving_settings
    ...
    assert alike["score"] == 64.0 and alike["projected_gold_at_15"] == 250.0
    assert off_role["score"] == 36.0 and off_role["projected_gold_at_15"] == -250.0
...
def test_gold_at_15_is_left_out_until_the_scores_carry_the_fit_spread(serving_settings):
    # given
    path = serving_settings.model_dir / "interaction_scores.npz"
    with np.load(path, allow_pickle=True) as data:
        saved = {name: data[name] for name in data.files if name != "gold_sd"}
    np.savez(path, **saved)
    ...
    # then
    assert found["projected_gold_at_15"] is None and found["score"] == 64.0
...
def test_a_lineup_sums_its_ten_pairs_and_ranks_against_corpus_teams(serving_settings):
    ...
    assert 0.0 < result["score"] < 100.0 and result["projected_gold_at_15"] == round(result["synergy"] * 5000.0, 2)
